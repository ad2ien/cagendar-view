...
export function CalendarYearView({ singleDayEvents, multiDayEvents }: IProps) {
  ...
  const MONTHS = Array.from({ length: 12 }, (_, i) => format(new Date(0, i, 1), "MMMM", { locale: getLocale() }));
                    ...
                    >
                      {isCurrentMonth && hasEvents ? (
                        ...
                        <div className="w-full h-full flex flex-col items-center justify-start">
                          <span className={cn("size-5 flex items-center justify-center font-medium")}>{cell.day}</span>
                        </div>
                      )}
                    </div>
                  );
